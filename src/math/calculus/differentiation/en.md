...
::: task
If a toy is sold at €130, it is purchased by 1000 people. For each euro that increases this price, the number of buyers decreases by 50, respectively.

1. Make a graph of the number of toys that are sold i work the sales price and give the formula that expresses it.
2. The cost price of a toy is € 80. Calculate the price $p$, which gives a maximum total benefit.
3. Find the number of toys that are sold if the price is $p$ and calculate the maximum benefit.

:::

::: task
The tangent line to parabola $y = 3 - x^2$ at a point $M$ located within the first quadrant $(x > 0, y > 0)$, cuts the axis $OX$ at the point $A$ and axis $OY$ in point $B$.

1. Make a graph of the elements of the problem.
2. Find the coordinates of the point $M$ that make the triangle $OAB$ have the minimum area.

::: task
Given the function $f(x) = \sqrt{x}$ and the point $A(2, 0)$ located on the axis of the abscissaes.

1. Find the function that expresses the distance from point $A$ to any point of the function graph.
2. Find the coordinates of the point of $f(x)$ closest to $A$.
